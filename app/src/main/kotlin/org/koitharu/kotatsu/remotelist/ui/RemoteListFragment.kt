...
class RemoteListFragment : MangaListFragment(), FilterCoordinator.Owner {
    ...
    /** Set when the error screen's "Open WebView" button was used; consumed on return. */
    ...
    override fun onSecondaryErrorActionClick(error: Throwable) {
        if (openInBrowser(error.getCauseUrl())) {
            // The WebView shares its cookies with the app, so a captcha solved there is already
            // valid. Reload the list when the user comes back instead of leaving the old error
            // (and its "solve" button) on screen until the app is restarted.
            retryOnResume = true
        }
    }
