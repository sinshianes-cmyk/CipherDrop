...
class WebtoonImageView @JvmOverloads constructor(
	...
	override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
		...
		if (isReady && w > 0 && h > 0 && sWidth > 0 && (w != oldw || h != oldh)) {
			// The parent resized this view (page measured, zoom changed). The library re-centers from a
			// mix of the old translate and the new size, which leaves the page shifted inside the view -
			// blank space above it and its bottom cut off. Re-apply the scroll for the new size.
