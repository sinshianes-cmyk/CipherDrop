...
class SuggestionsWorker @AssistedInject constructor(
	...
	private suspend fun getList(
		...
		// A slow or hung source must not hold one of the few parallel slots for minutes.
		...
		if (e is CloudFlareException) {
			// Record the protection state only. Notifying here fired one "captcha required"
			// notification (plus a favicon download) per blocked source, while still holding a
			// scan slot - very slow with many sources, and noise for a background job.
			captchaHandler.handle(e, tryAutoResolve = false, notify = false).let { Unit }
		}
		e.printStackTraceDebug()
